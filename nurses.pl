...
writeClauses:-
    nurseStartsExacly1, % una infermera comença una sola vegada
	nurseHaveOneType, % el que diu el nom.
	nurseNotWorksinRange, % una infermera no treballa al rang x-y.
	hora_x_nurses, 
	nurseMin,
	nurseMax,
	nurseInter,
    ...
    nurseStartsExacly1:- nurse(N), findall( startsNH-N-H, hour(H) , Lits ), exactly(1, Lits), fail.
    nurseStartsExacly1.

    nurseHaveOneType:- nurse(N), findall( nurseType-N-Type, type(Type), Lits), exactly(1, Lits), fail.
    nurseHaveOneType.

    nurseNotWorksinRange:- nurse(N), nurseIDandBlocking(N, X, Y), findall( worksNH-N-H, hourInRange(X, Y, H) , Lits), negateAll(Lits, NLits), writeClause(NLits), fail.
	nurseNotWorksinRange.

	hora_x_nurses:- hour(H), needed(H, NursesNeeded), findall(worksNH-N-H, nurse(N), Lits), atLeast(NursesNeeded, Lits), fail.
	hora_x_nurses.

	nurseMin:- nurse(N), hour(SH), type(T), workingHourForTypeAndStartH(T, SH, H), negate(startsNH-N-SH, Stneg), w(Stneg), negate(nurseType-N-T, Tneg), w(Tneg), writeClause([worksNH-N-H]), fail.
	nurseMin.
